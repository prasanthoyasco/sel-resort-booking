...
export default function FilterBar() {
  ...
  const filterSummary = () => {
    ...
    if (filters.price) {
      parts.push(
        `within the range of Max ₹ ${filters.price}`
      );
    }
  ...
  return (
    <div ref={wrapperRef} className="relative z-40 flex items-center gap-3">
      {/* All filters */}
      <button className="relative flex items-center gap-2 px-4 py-2 rounded-full border border-indigo-600 text-sm font-medium">
        <span
          onClick={() => {
            if (activeCount > 0) {
              setFilters({
      ...
      <div className="relative">
        ...
        >
          ...
          {filters.propertyTypes.length > 0 && (
            <span className="count-badge">{filters.propertyTypes.length}</span>
          )}
        </button>
